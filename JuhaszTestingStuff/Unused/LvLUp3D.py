...
def GetImage(i, depth = False):
    if(depth == True):
        img = cv2.imread(depth_images_folder+f"/Processed_{i}.jpg")
        
        if(img is None):
            print(f"GetImage {depth_images_folder}/Processed_{i}.jpg picture missing!")
        else:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
        img = cv2.imread(valid_images_folder+f"/{i}.jpg")

        if(img is None):
            print(f"GetImage {valid_images_folder}/{i}.jpg picture missing!")
        else:
...
#depth_image = GetImage(i, True)


#get the image from the folder
depth_image = GetImage(i, True)
color_image = GetImage(i, False)

'''
for i in range(Depth.num_samples):
    cloud = ThreeDUtils.create_point_cloud(depth_image, color_image)
...
print("shapes: " , depth_image.shape, " ", color_image.shape)
#width, height = depth_image.shape[0], depth_image.shape[1]
width, height = depth_image.shape
#width, height = Depth.samples[0].shape[0], Depth.samples[0].shape[1]
...
#camera_intrinsic1 = ThreeDUtils.get_intrinsics(height, width, 100) 
#fx, fy = camera_intrinsic1[0, 0], camera_intrinsic1[1, 1]
#cx, cy = camera_intrinsic1[0, 2], camera_intrinsic1[1, 2]
#camera_intrinsic1 = o3d.camera.PinholeCameraIntrinsic(width, height, fx, fy, cx, cy)

camera_intrinsic = o3d.camera.PinholeCameraIntrinsic()
camera_intrinsic.set_intrinsics(width, height, 300, 300, width/2, height/2)

pcd_raw = o3d.geometry.PointCloud.create_from_rgbd_image(rgbd_image, camera_intrinsic)
#pcd_2 = ThreeDUtils.create_point_cloud(depth_image, color_image, camera_intrinsic1)

print("shape: ", np.asarray(pcd.points).shape)
print("first values: ", np.asarray(pcd.points)[200:210])

o3d.visualization.draw_geometries([pcd])

#o3d.visualization.draw_geometries([pcd, ThreeDUtils.create_point_cloud(depth_image, color_image, custom_intrinsic)])
