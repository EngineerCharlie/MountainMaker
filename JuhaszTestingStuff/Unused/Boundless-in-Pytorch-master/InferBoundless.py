...
def CreateInferation(pil_image_rgb, num):
...
def CreateInferModelInferation(width_factor=3):
    ...
    filename = "15.jpg"
    fd = f"C:/WorkingSets/3D/OriginalImages/{filename}"
    ...
    final_right = CreateInferation(pil_image_rgb, width_factor)
    final_right = np.asarray(final_right)


    image_mirrored = ImageOps.mirror(pil_image_rgb)
    final_left = CreateInferation(image_mirrored, width_factor)
