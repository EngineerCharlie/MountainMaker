...
def define_generator(image_shape=(192, 256, 3)):
    # weight initialization
    init = RandomNormal(stddev=0.02)
    # image input
...
def gen_mountains(g_model_path, input_image_path, output_image_path="plot_genned_mountain.png"):
    ...
    X_realA = load_single_image(input_image_path)
    X_realA = expand_dims(X_realA, axis=0)
    # Generate a batch of fake samples
    X_fakeB = g_model.predict(X_realA)
    # Scale pixel values from [-1,1] to [0,1]
    X_realA = (X_realA + 1) / 2.0
    X_fakeB = (X_fakeB + 1) / 2.0
    # Plot input image and generated output
